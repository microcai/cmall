...
#  define HTTPD_RECEIVE_BODY_MAX       (200 * 1024 * 1024)
...
namespace cmall
{
	...
	*/
	...
	public:
		...
		// 异步执行一个url请求, 请求参数由req指定, 请求返回通过error_code 或 http_response
		// 得到, 如果发生错误, 则会得到error_code, 若请求正常则返回http_response.
		// 这个版本可指定sock5 proxy, 如 socks5://127.0.0.1:1080
		template<class Handler>
		BOOST_ASIO_INITFN_RESULT_TYPE(Handler, void(boost::system::error_code, http_response))
...
#ifdef SIMPLE_HTTP_USE_FLAT_BUFFER
			beast::flat_buffer b(HTTPD_RECEIVE_BODY_MAX);
			b.reserve(HTTPD_RECEIVE_BUFFER_SIZE);
#else

...
namespace cmall
{
	...
	public:
		...
		void start()
		{
			...
			boost::asio::spawn(executor, [this](boost::asio::yield_context yield) { do_read(yield); });
			boost::asio::spawn(executor, [this](boost::asio::yield_context yield) { do_write(yield); });
		}
		...
		void register_hander(std::function<message_handler>&& handler) { sig_conn_ = got_message.connect(handler); }

		void reply(const std::string& res) { queue_.emplace_back(res); }

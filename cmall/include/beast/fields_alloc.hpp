...
public:
    static
    static_pool&
    construct(void* pre_alloc, std::size_t size)
    {
        return *(::new(pre_alloc) static_pool{size - sizeof (static_pool)});
    }
...
struct fields_alloc
{
    alloc_detail::static_pool* pool_;
...
public:
    ...
    explicit
    fields_alloc(char* preallocated, std::size_t size)
        : pool_(&alloc_detail::static_pool::construct(preallocated, size))
    {
    }
